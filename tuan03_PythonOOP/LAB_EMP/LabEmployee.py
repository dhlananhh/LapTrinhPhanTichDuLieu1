...
# Khai báo biến lưu trữ các nhân viên
dsNhanVien = []
...
while (True):
    ...
    if userChoice == 1:
        ...
        fr = open(_currentDir + '/python tuan 3/LAB_EMP/dbemp_input.db', mode='r', encoding='utf-8')
        for line in fr:
    ...
    # Hiển thị thông tin của một nhân viên khi biết mã nhân viên
    elif userChoice == 4:
        # nhập id
        maNV = input("Input code: ")
        ktNV = 0
        for item in dsNhanVien:
            if item.code == maNV:
                item.display()
    ...
    # Chỉnh sửa thông tin một nhân viên
    elif userChoice == 5:
        # nhập id
        maNV = input("Input code: ")
        ktNV = 0
    ...
    # Xóa một nhân viên ra khỏi danh sách
    elif userChoice == 6:
        # nhập id
        maNV = input("Input code: ")
        ktNV = 0
    ...
    # Tăng lương của một nhân viên
        ...
        if ktNV == 0:
            print("Nothing here")
        if ktNV == 1:
            luongMoi = float(input("Input amount: "))
            for item in dsNhanVien:
                if item.code == maNV:
                    item.salary += luongMoi

    # Giảm lương của một nhân viên
    elif userChoice == 8:
        # nhập id
        maNV = input("Input code: ")
        ktNV = 0
    ...
    # Tính số lượng nhân viên (countEmp) và xuất ra màn hình
    elif userChoice == 9:
        print('Số lượng nhân viên trong công ty là: ', len(dsNhanVien))
    
    # Tính tổng tiền lương mà công ty phải trả hàng tháng (sumSalary) và xuất ra màn hình
    elif userChoice == 10:
        sumSalary = 0.0
        for item in dsNhanVien:
            sumSalary += item.salary
        print(f'Tổng tiền lương mà công ty phải trả hàng tháng là: {sumSalary:.2f}')
    
    # Tính trung bình lương của nhân viên (avgSalary) và xuất ra màn hình
    elif userChoice == 11:
        sumEmp = len(dsNhanVien)
        # tính tổng lương
        sumSalary = 0.0
        for item in dsNhanVien:
            sumSalary += item.salary
        # tính trung bình lương
        avgSalary = sumSalary / sumEmp
        print(f'Trung bình lương của nhân viên là: {avgSalary:.2f}')
    
    # Tính độ tuổi trung bình của nhân viên (avgAge) và xuất ra màn hình
    elif userChoice == 12:
        sumEmp = len(dsNhanVien)
        sumAge = 0.0
        for item in dsNhanVien:
    ...
    # Tính tuổi lớn nhất của các nhân viên (maxAge) và hiển thị danh sách nhân viên có tuổi lớn nhất
    elif userChoice == 13:
    ...
    # Vẽ biểu đồ tương quan lương theo độ tuổi
    elif userChoice == 15:
        arrTuoi = []
        arrLuong = []
        for item in dsNhanVien:
    ...
    # Vẽ biểu đồ so sánh lương trung bình của các nhóm tuổi: nhỏ hơn 35, từ 35 đến 50, hơn 50 trở lên
    elif userChoice == 16:
    ...
    # Vẽ biểu đồ thể hiện phần trăm tổng lương trên các nhóm tuổi như Opt-16
    elif userChoice == 17:
        arrAgeGroup = ['less than 35', 'from 35 to 50', 'more than 50']
        arrAvgSalary = [0.0, 0.0, 0.0]
        noiBat = [0, 0.2, 0]
    ...
    # Vẽ biểu đồ thể hiện phần trăm số lượng nhân viên theo các nhóm tuổi như Opt-16 
    elif userChoice == 18:
        arrAgeGroup = ['less than 35', 'from 35 to 50', 'more than 50']
        arrCount = [0.0, 0.0, 0.0]
        noiBat = [0, 0.2, 0]
    ...
    # Lưu danh sách nhân viên xuống file dbemp_output.db, biết rằng mỗi nhân viên là một dòng và các thông tin nhân viên được phân cách bởi dấu '-'
    elif userChoice == 19:
        fw = open(_currentDir + '/python tuan 3/LAB_EMP/emp_output.db', mode='r', encoding='utf-8')
        for item in dsNhanVien:
            fw.write(
